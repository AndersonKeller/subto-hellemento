...
import { uuseConfigsStore } from "./stores/configs.store";
// import favicon
...
if (data.value.styles) {
  if (import.meta.client) {
    ...
    // style.innerHTML = data.value.styles;
    let content = "";
    for (const key in data.value.styles) {
      content += `${key}:${data.value.styles[key]};`;
    }
...
useHead(() => ({
  title: `${data.value.titulo} | ${route.meta.title}`,
